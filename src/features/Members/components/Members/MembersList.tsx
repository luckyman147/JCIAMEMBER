...
export default function MembersList({ members, loading, sortBy }: MembersListProps) {
    ...
    const groupedMembers = React.useMemo(() => {
        ...
        // Populate groups preserving member order (which is sorted by parent)
        ...
        const entries = Object.entries(groups).filter(([role]) => !['admin'].includes(role.toLowerCase()));

        // If a sort is active, order groups by their highest performer
        if (sortBy && sortBy !== 'default') {
            return entries.sort((a, b) => {
                const aMax = Math.max(...a[1].map(m => {
                    if (sortBy.includes('jps')) return m.jps_score || 0;
                    if (sortBy.includes('volunteering')) return m.estimated_volunteering_hours || 0;
                    return 0;
                }), 0);
                const bMax = Math.max(...b[1].map(m => {
                    if (sortBy.includes('jps')) return m.jps_score || 0;
                    if (sortBy.includes('volunteering')) return m.estimated_volunteering_hours || 0;
                    return 0;
                }), 0);

                if (sortBy.endsWith('_asc')) return aMax - bMax;
                return bMax - aMax;
            });
        }

        // Default: Sort roles alphabetically
                                        ...
                                        >
                                            ...
                                            <td className="px-6 py-4 whitespace-nowrap">
                                                <div className="flex items-center">
                                                    <div className="h-14 w-14 shrink-0 rounded-full bg-gray-200 flex items-center justify-center overflow-hidden border-2 border-white shadow-sm">
                                                        {member.avatar_url ? (
